...
public class ProductionManager implements AcceptsSupplies {
    ...
    public ProductionManager(Developments developments, MutableProduction baseProduction, LeadersSpace leadersSpace) {
        containers.put(DepotID.SourceType.DEPOT, new SupplyContainer(SupplyContainer.AcceptStrategy.onlyFrom(DepotID.SourceType.DEPOT, leadersSpace)));
        containers.put(DepotID.SourceType.STRONGBOX, new SupplyContainer(SupplyContainer.AcceptStrategy.onlyFrom(DepotID.SourceType.STRONGBOX, leadersSpace)));
    ...
    public void addSupply(DepotID slot, WarehouseObjectType wot, DepotID from) throws SupplyException {
        ...
        if (slot.getType(leadersSpace) == DepotID.DepotType.DEVELOPMENT){
            ...
            System.exit(1); /*TODO terminate program*/
        }
    }
    ...
    public void removeSupply(DepotID slot, WarehouseObjectType wot, DepotID to) throws SupplyException {
        ...
        try {
            ...
            e.printStackTrace();System.exit(1); /*TODO terminate the program because we checked before, so everything should go smooth*/}
    }
            ...
            if (wot != WarehouseObjectType.NO_TYPE) {
                for (int i = 0; i < toRemove.getQuantity(wot); ++i) {
                    try {
                        ...
                        } catch (SupplyException se1) {System.exit(1); /*TODO terminate program*/}
                    }
                }
            }
        }
